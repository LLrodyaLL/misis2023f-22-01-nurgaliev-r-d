...
void Program::Render() {
...

    {
        ...
        if (firstImage.isOpened && secondImage.isOpened) {

            cv::resize(firstImage._data, firstImage._data, secondImage._data.size());

            // Наложение изображений
            overlayImage.overlay(firstImage, secondImage, 0.5);

            // Обновление текстуры OpenGL после наложения
            overlayImage.updateGLTexture();

            // Преобразование cv::Mat обратно в изображение ImDrawList
            ImDrawList* drawList = ImGui::GetWindowDrawList();
